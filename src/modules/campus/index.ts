 ...
 *
 * Manages physical campus locations and their bookable facilities.
 *
 ...
 *
 * IMPORTANT: All static sub-resource routes (/facilities/*, /bookings/*)
 * are registered BEFORE the dynamic /:id route to prevent Hono matching
 * '/facilities' as an :id parameter.
 *
 * Routes:
 *   POST   /api/campus                          — Create campus
 *   GET    /api/campus                          — List campuses
 *
 *   POST   /api/campus/facilities               — Create facility
 *   GET    /api/campus/facilities               — List facilities
 *   GET    /api/campus/facilities/:id           — Get facility
 *   PATCH  /api/campus/facilities/:id           — Update facility
 *
...
campusRouter.post('/facilities', requireRole(['admin']), async (c) => {
  const tenantId = c.get('user').tenantId;
  const body = await c.req.json<{
    campusId?: string;
    name: string;
...
campusRouter.get('/facilities', requireRole(['admin', 'staff', 'teacher', 'student']), async (c) => {
  const tenantId = c.get('user').tenantId;
  const { campusId, type, status } = c.req.query() as Record<string, string>;

  let sql = 'SELECT * FROM facilities WHERE tenantId = ?';
...
campusRouter.get('/facilities/:id', requireRole(['admin', 'staff', 'teacher', 'student']), async (c) => {
  const tenantId = c.get('user').tenantId;
  const id = c.req.param('id');
  const facility = await c.env.DB.prepare(
    'SELECT * FROM facilities WHERE id = ? AND tenantId = ?'
  ).bind(id, tenantId).first();
  if (!facility) return c.json({ error: 'Facility not found' }, 404);
  return c.json({ data: facility });
});

campusRouter.patch('/facilities/:id', requireRole(['admin']), async (c) => {
  const tenantId = c.get('user').tenantId;
  const id = c.req.param('id');
  const body = await c.req.json<{
...
campusRouter.post('/bookings', requireRole(['admin', 'staff', 'teacher']), async (c) => {
  ...
  const facility = await c.env.DB.prepare(
    `SELECT id, status FROM facilities WHERE id = ? AND tenantId = ?`
  ...
  const conflict = await c.env.DB.prepare(
    `SELECT id FROM facilityBookings
...
campusRouter.get('/bookings', requireRole(['admin', 'staff', 'teacher']), async (c) => {
  const user = c.get('user');
  const tenantId = user.tenantId;
  const { facilityId, status } = c.req.query() as Record<string, string>;

  let sql = 'SELECT * FROM facilityBookings WHERE tenantId = ?';
  const args: unknown[] = [tenantId];
...
campusRouter.get('/bookings/:id', requireRole(['admin', 'staff', 'teacher']), async (c) => {
  const tenantId = c.get('user').tenantId;
  const id = c.req.param('id');
  const booking = await c.env.DB.prepare(
    'SELECT * FROM facilityBookings WHERE id = ? AND tenantId = ?'
  ).bind(id, tenantId).first();
...
campusRouter.patch('/bookings/:id/cancel', requireRole(['admin', 'staff', 'teacher']), async (c) => {
  const user = c.get('user');
  const tenantId = user.tenantId;
  const id = c.req.param('id');

  const booking = await c.env.DB.prepare(
    'SELECT id, bookedBy, status FROM facilityBookings WHERE id = ? AND tenantId = ?'
  ...
  const now = new Date().toISOString();
  await c.env.DB.prepare(
    `UPDATE facilityBookings SET status = 'cancelled', updatedAt = ? WHERE id = ? AND tenantId = ?`
