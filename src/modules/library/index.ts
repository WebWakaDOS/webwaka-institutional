...
libraryRouter.post('/inst_books', requireRole(['admin', 'librarian']), async (c) => {
...
libraryRouter.get('/inst_books', requireRole(['admin', 'librarian', 'student', 'inst_staff']), async (c) => {
  const tenantId = c.get('user').tenantId;
  const { category, search } = c.req.query() as Record<string, string>;
  let sql = 'SELECT * FROM inst_books WHERE tenantId = ?';
...
libraryRouter.post('/checkout', requireRole(['admin', 'librarian']), async (c) => {
  ...
  const book = await c.env.DB.prepare(
    'SELECT id, availableCopies FROM inst_books WHERE id = ? AND tenantId = ?'
  ...
  await c.env.DB.prepare(
    `INSERT INTO inst_bookCheckouts
  ...
  await c.env.DB.prepare(
    'UPDATE inst_books SET availableCopies = availableCopies - 1, updatedAt = ? WHERE id = ? AND tenantId = ?'
...
libraryRouter.patch('/:id/return', requireRole(['admin', 'librarian']), async (c) => {
  ...
  const checkout = await c.env.DB.prepare(
    'SELECT bookId, dueDate, returnedAt FROM inst_bookCheckouts WHERE id = ? AND tenantId = ?'
  ...
  await c.env.DB.prepare(
    'UPDATE inst_bookCheckouts SET returnedAt = ?, lateFeeKobo = ? WHERE id = ? AND tenantId = ?'
...
libraryRouter.get('/checkouts', requireRole(['admin', 'librarian']), async (c) => {
  ...
  let sql = 'SELECT * FROM inst_bookCheckouts WHERE tenantId = ?';
  const args: unknown[] = [tenantId];
