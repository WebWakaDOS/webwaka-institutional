 ...
 *
 * Stores patient medical histories, prescriptions, and lab results.
 *
 ...
 *
 *   POST  /api/ehr/patients/:id/prescriptions      — Add prescription
 *   GET   /api/ehr/patients/:id/prescriptions      — List prescriptions
 *   PATCH /api/ehr/prescriptions/:id/status        — Update prescription status
 *
...
ehrRouter.post('/patients', requireRole(['admin', 'doctor', 'nurse']), async (c) => {
  ...
  const existing = await c.env.DB.prepare(
    'SELECT id FROM patientRecords WHERE patientId = ? AND tenantId = ?'
...
ehrRouter.get('/patients', requireRole(['admin', 'doctor', 'nurse']), async (c) => {
  ...
  const { results } = await c.env.DB.prepare(
    'SELECT * FROM patientRecords WHERE tenantId = ? ORDER BY createdAt DESC'
  ).bind(tenantId).all();
...
ehrRouter.get('/patients/:id', requireRole(['admin', 'doctor', 'nurse']), async (c) => {
  ...
  const record = await c.env.DB.prepare(
    'SELECT * FROM patientRecords WHERE id = ? AND tenantId = ?'
...
ehrRouter.post('/patients/:id/prescriptions', requireRole(['admin', 'doctor']), async (c) => {
  const tenantId = c.get('user').tenantId;
...
ehrRouter.get('/patients/:id/prescriptions', requireRole(['admin', 'doctor', 'nurse']), async (c) => {
  const tenantId = c.get('user').tenantId;
  const patientId = c.req.param('id');
  const { results } = await c.env.DB.prepare(
    'SELECT * FROM prescriptions WHERE tenantId = ? AND patientId = ? ORDER BY prescribedAt DESC'
  ).bind(tenantId, patientId).all();
  return c.json({ data: results });
});

ehrRouter.patch('/prescriptions/:id/status', requireRole(['admin', 'doctor']), async (c) => {
  const tenantId = c.get('user').tenantId;
  const id = c.req.param('id');
  ...
  await c.env.DB.prepare(
    'UPDATE prescriptions SET status = ? WHERE id = ? AND tenantId = ?'
...
ehrRouter.get('/patients/:id/labs', requireRole(['admin', 'doctor', 'nurse']), async (c) => {
  ...
  const patientId = c.req.param('id');
  const { results } = await c.env.DB.prepare(
    'SELECT * FROM labResults WHERE tenantId = ? AND patientId = ? ORDER BY testedAt DESC'
  ).bind(tenantId, patientId).all();
  return c.json({ data: results });
});
