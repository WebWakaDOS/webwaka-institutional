/**
 * Paystack Integration — WebWaka Institutional Suite
 *
 ...
 *
 * Use cases: School fees, hostel fees, exam fees, library fees
 */
...
/**
 * Generate a unique Paystack payment reference for institutional fees.
 * Format: INS-{tenantId}-{timestamp}-{random}
 */
export function generatePaymentReference(tenantId: string): string {
  const random = Math.random().toString(36).slice(2, 7);
  return `INS-${tenantId.slice(0, 8)}-${Date.now()}-${random}`;
}
